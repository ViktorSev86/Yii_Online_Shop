...
class MenuWidget extends Widget
{
    ...
    public function run()
    {
        ...
        \Yii::$app->cache->set('menu', $this->menuHtml, 60); // Кэшируем данные (на 60 секунд, чтобы проверить, что они берутся из кэша. Если время не указано, кэш хранится неограниченно долго. При добавлении категории необходимо предусмотреть функцию очистки кэша, чтобы новая категория появилась в меню сразу после добавления)
    ...
    protected function getMenuHtml($tree) { // Свойство tree передаётся в виде параметра (а не используется просто в виде свойства), потому что мы будем вызывать рекурсивно из шаблона в методе catToTemplate() не всё дерево, а каждую конкретную категорию этого дерева
        $str = '';
